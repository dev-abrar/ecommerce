...
// Select all swiper-slide images
const slides = document.querySelectorAll('.mySwiper2 .swiper-slide img');
const zoomContainer = document.querySelector('.zoom-preview');
const zoomedImg = document.getElementById('zoomed_img');

slides.forEach(slide => {
	slide.addEventListener('mouseenter', function () {
		zoomedImg.src = this.src; // Set zoomed image
	...
	slide.addEventListener('mousemove', function (e) {
		...
		// Move zoomed image so the area under cursor is visible
		zoomedImg.style.transform = `translate(-${xPercent}%, -${yPercent}%) scale(2)`;
	});

	slide.addEventListener('mouseleave', function () {
		zoomContainer.style.display = 'none'; // hide on leave
	});
});
...
document.querySelectorAll('.qty-selector').forEach(selector => {
	...
	const updateButtons = () => {
		...
		btnDecrease.disabled = val <= minQty;
		btnIncrease.disabled = val >= maxQty;
	}
